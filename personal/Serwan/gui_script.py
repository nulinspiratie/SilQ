...
if __name__ == "__main__":
    ...
    from silq.gui import start_gui
            ...
            if not self.mini:
                ...
                for k, (color, keys) in enumerate([
                    ...
                    for (key, pos) in keys:
                        ...
                        key_palette.setColor(QPalette.Foreground,
                                             getattr(Qt, color))
    ...
    class SIMControlDialog(QDialog):
        ...
        def _copy_from_clipboard(self, *args):
            ...
            for parameter_name, val in clipboard_dict.items():
                self.SIM928_dialogs[parameter_name].val_textbox.setText(
                    str(val))
            ...
            if voltage is not None:
                ...
                for SIM928_dialog in self.SIM928_dialogs.values():
                    if SIM928_dialog.modified_val:
                        SIM928_dialog.set_voltage(
                            SIM928_dialog.val_textbox.text())
    ...
    from silq import parameters
    voltage_parameters_raw = [ManualParameter(name, initial_value=0.5)
                              for name in ['TGAC', 'TG', 'DF', 'DS']]
    voltage_parameters = [parameters.ScaledParameter(p, scale=8)
                          for p in voltage_parameters_raw]
